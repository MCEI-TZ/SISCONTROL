...
function buscar(page = 1) {
  ...
  axios
    .get(`${generalDate}/api/alumnos/?page=${page}`,{
      headers: { Authorization: `Bearer ${token}` },
    })
    .then((response) => {
...
function buscarId() {
  ...
  const url = `${generalDate}/api/alumnos/${searchInput}`;
  ...
  apiClient
    ...
    .catch((error) => {
      ...
      mostrarMensaje("Error al buscar alumno. Por favor, intente de nuevo.");
    });
}
...
async function renderResultados(resultados) {
  ...
  for (const result of resultados) {
    const carreraNombre = await ObtenerCarrera(result.IdCarre);
    ...
    resultDiv.innerHTML = `
        <div class="card-body card border-primary">
          <div class="row">
              <div class="col-1 card-title fs-7 text-warning-emphasis text-center border-bottom border-primary mb-2">
                  N.Control
              </div>
              <div class="col-2 card-title fs-7 text-warning-emphasis text-center border-bottom border-secondary-subtle mb-2">
                  Nombre
              </div>
              ...
              <div class="col-2 card-title fs-7 text-warning-emphasis text-center border-bottom border-success-subtle mb-2">
                Carrera
              </div>
              <div class="col-2 card-title fs-7 text-warning-emphasis text-center border-bottom border-warning-subtle mb-2">
                  Email
              </div>
              <div class="col-2 card-title fs-7 text-warning-emphasis text-center border-bottom border-danger mb-2">
                  Nacimiento
              </div>
          </div>
          <div class="row">
              ...
              <div class="col-2 text-center mb-2">
                ${result.CorreoE == null ? "No otorgo" : result.CorreoE}
              </div>
              <div class="col-2 text-center mb-2">
                ${formatDate(result.FNacimiento)}
              </div>
          </div>
      `;
...
async function ObtenerCarrera(id) {
  try {
    ...
    const res = await apiClient.get(`${generalDate}/api/carrera/${id}`);

    // Extract the name of the event from the API response
    const { NombreCorto } = res.data;

    // Return the name of the event
    return NombreCorto;
  } catch (error) {
    // Log the error to the console
    console.error("Error al obtener el carrera:", error);
